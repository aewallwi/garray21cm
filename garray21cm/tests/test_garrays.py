...
def test_generate_1d_array():
    ...
    xyz1 = garrays.generate_1d_array(
        antenna_count=4, separation_unit=2.0, angle_EW=0.0, copies=1, copy_separation=2.0
    )
...
def test_initialize_telescope_yaml(tmpdir):
    ...
    antpos = garrays.generate_1d_array(antenna_count=4, separation_unit=2.0, angle_EW=np.pi/3., copies=2, copy_separation=2.0)
    (
        obs_param_yaml_name,
        telescope_yaml_name,
        csv_name,
